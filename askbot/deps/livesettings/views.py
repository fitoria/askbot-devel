...
from askbot.deps.livesettings import FileValue
from askbot.deps.livesettings.overrides import get_overrides
import logging

log = logging.getLogger('configuration.views')

def _get_bases(instance):
    '''returns a list of base clases from
    an instance'''
    return [c.__name__ for c in instance.__class__.__bases__]
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                for name, value in form.cleaned_data.items():
                    ...
                    #detecting if a FileValue is the parent of the setting
                    if isinstance(cfg, FileValue) or 'FileValue' in _get_bases(cfg):
